...
public class ForgetSpell extends CommandSpell {
    ...
    public PostCastAction castSpell(Player player, SpellCastState state, float power, String[] args) {
        if (state == SpellCastState.NORMAL) {
            ...
            Spellbook casterSpellbook = MiracleSpell.getSpellBook(player).observe();
            if (casterSpellbook == null) {
                player.sendMessage("§6MiracleSpell §7| §7Player data is loading in progress.");
                return PostCastAction.ALREADY_HANDLED;
            }
            ...
            ObservableFuture<Spellbook> spellBookObserver = MiracleSpell.getSpellBook(target);
            if (!spellBookObserver.isDone()) {
                player.sendMessage("§6MiracleSpell §7| §7Target player data is loading in progress. Delayed...");
            }
            spellBookObserver.reserve(createAction(player, target, spell, args, all));
    ...
    private Consumer<ObservableFuture<Spellbook>> createAction(Player player, Player target, Spell spell, String[] args, boolean all) {
        return (future) -> {
            Spellbook targetSpellbook = future.observe();
    ...
    public boolean castFromConsole(CommandSender sender, String[] args) {
        if (args == null || args.length != 2) {
            ...
            if (target == null) {
                ...
                ObservableFuture<Spellbook> spBook = MiracleSpell.getSpellBook(target);
                spBook.syncReserve((book) -> {
                    Spellbook targetSpellbook = book.observe();
                    Spell spell = null;
                    boolean all = false;
                    if (args[1].equals("*")) {
                        all = true;
